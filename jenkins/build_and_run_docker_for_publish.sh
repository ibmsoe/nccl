...
# DOCKER_RUN_SCRIPT - Script to run under docker (relative to nccl repo root)
...
# into the same dir as nccl (git_root). The container will
...
docker run --pid=host \
  ...
  -v "$git_root:/var/local/jenkins/nccl:ro" \
  -v $CCACHE_DIR:$CCACHE_DIR \
  -w /var/local/git/nccl \
  --name=$CONTAINER_NAME \
  $DOCKER_IMAGE_NAME \
  bash -l "/var/local/jenkins/nccl/$DOCKER_RUN_SCRIPT" || FAILED="true"

# Copy output artifacts
if [ "$OUTPUT_DIR" != "" ]
then
  docker cp "$CONTAINER_NAME:/var/local/git/nccl/$OUTPUT_DIR" "$git_root" || FAILED="true"
fi
